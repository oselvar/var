...
<Tabs syncKey="lang">
  ...
  <TabItem label="Ruby">
    ```ruby
    sensor.call('the alarm fired') { |state| raise 'no alarm' unless state[:alarm] }
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="lang">
  ...
  <TabItem label="Ruby">
    ```ruby
    param, stimulus, sensor = steps { { dice: [5, 6] } }
    param.call('numbers', '\d+(?:, \d+)*', parse: ->(raw) { raw.split(', ').map(&:to_i) })

    # "The dice show 5, 6" — {numbers} transforms to [5, 6]
    sensor.call('The dice show {numbers}') { |state, _dice| state[:dice] }
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="lang">
  ...
  <TabItem label="Ruby">
    ```ruby
    sensor.call('I should have {int} cukes in my {word} belly') do |state, _count, _belly|
...
<Tabs syncKey="lang">
  ...
  <TabItem label="Ruby">
    ```ruby
    sensor.call('Greet {word}:') { |_state, name, _doc| [name, "Hello, #{name}!\n"] }
    ```
  </TabItem>
</Tabs>
